...
- 当前回归基线: main HEAD **4091ece**（[#106](https://github.com/lync-cyber/intelli-source/pull/106) ~ [#126](https://github.com/lync-cyber/intelli-source/pull/126) 已合并，闭环详情见 [HISTORY](docs/HISTORY-intellisource-v1.md) / [BACKLOG](docs/BACKLOG-intellisource-v1.md)）；最近合并 [PR #126](https://github.com/lync-cyber/intelli-source/pull/126) 门禁绿。当前会话 branch `feat/b074-remote-infra`（B-074 ②③ 远端 infra：Bash 置备脚本 + systemd 模板 + GHCR registry 推送工作流 + compose pull override + deploy-spec/remote-host-readiness 同步）待 commit/PR；门禁面：**无 Python/src/tests 改动（纯 infra/docs）**，故 Python 质量门禁无回归面，本地验证 compose config + bash -n + YAML 全 PASS、镜像名 CI·compose·文档三处一致。**待办**：用户本机 Docker registry-mirror 已改 `~/.docker/daemon.json`（删 403 的 `docker.xuanyuan.me`、留 `docker.m.daocloud.io`，备份 `daemon.json.bak.20260618`），待用户重启 Docker 引擎生效
