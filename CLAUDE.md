...
- 当前回归基线: main HEAD 9554c8b（[PR #106](https://github.com/lync-cyber/intelli-source/pull/106) ~ [PR #115](https://github.com/lync-cyber/intelli-source/pull/115) 已合并；#110 = cataforge 0.9.1 升级 + KG 摄取门禁修复，#111 = KG dangling WARN 处理，#113 = cataforge 0.11.0 升级，#115 = cataforge 0.11.2 升级 + KG 全量重建）。D1 Docker 缓存根治（PR #109 已合并）：Dockerfile `ARG GIT_SHA`+`LABEL` bust src 层（依赖层 CACHED 保留）+ compose build.args + `intellisource up`/`make up` 注入 sha + `--rebuild` 逃生口 + `test_stack.py` 9 用例；受控实验证伪（改 GIT_SHA+改 src+不带 --no-cache → 镜像新 src，uv sync CACHED）；code-review approved_with_notes（R-001~R-004 inline 闭环）；ruff+mypy --strict 267+全量 unit exit 0 绿。前次全门禁基线 integration 160 PASS/1 skip + lint-imports 12/12（本轮未重跑）
...
- 剩余项目级真债（非阻塞）: 详见 [BACKLOG](docs/BACKLOG-intellisource-v1.md)。开放项 — B-070/P2 Chat 会话压缩与写入截断未协调→AC-053 未兑现（修复待合并 [PR #118](https://github.com/lync-cyber/intelli-source/pull/118)）；B-071/P3 arch `[chat]` 配置段与实现失配（立项研判，B-070 follow-up）；P3 session-splitting 压缩设计(已评估 2026-06-16 NO-GO，重评触发见 BACKLOG)；P3 KG dangling WARN 76 个（TC/CR/SR 纯关系 class 框架口径噪声，已定性，修复点在 CataForge 框架侧，0.11.2 仍复现已聚焦 escalate [CataForge#292](https://github.com/lync-cyber/CataForge/issues/292)）
