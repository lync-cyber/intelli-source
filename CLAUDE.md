...
- 下一步行动: release 已放行，无阻塞项；剩余 backlog **P2 B-036 / P3 B-014 + B-016~B-019** 全部非阻塞（B-011 闭环 — 规约入双 COMMON-RULES + 清扫 46 处 truly-decorative，余 18 皆合法 guard；B-015 早已闭环 — promtool 在 CI；B-012 闭环 — keyword_tag 三缺陷修复；B-034 闭环 — walkthrough 全量订正）；B-014 需真 staging deploy 验证；BGE-M3 本地 embedding 暂缓
...
- Backlog 总入口: [docs/BACKLOG-intellisource-v1.md](docs/BACKLOG-intellisource-v1.md) — **release 已放行；剩余全部非阻塞** / P2: B-036 / P3: B-014 + B-016~B-019（B-011 已闭环）
