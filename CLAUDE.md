...
- 当前回归基线: main HEAD **4ee4dca**（[#106](https://github.com/lync-cyber/intelli-source/pull/106) ~ [#131](https://github.com/lync-cyber/intelli-source/pull/131) 已合并，闭环详情见 [HISTORY](docs/HISTORY-intellisource-v1.md) / [BACKLOG](docs/BACKLOG-intellisource-v1.md)）；最近合并 [#130](https://github.com/lync-cyber/intelli-source/pull/130)/[#131](https://github.com/lync-cyber/intelli-source/pull/131)（镜像 CVE 加固 + trivy 门禁 `ignore-unfixed: true` + DB gosu `skip-files`；B-074 ①②⑤ 经真实 CI 闭环）。无在途特性 PR，工作树干净且与 origin/main 同步。本机 Docker registry-mirror 仅留 `docker.m.daocloud.io`（403 源已删并重启复验；备份 `daemon.json.bak.20260618`）
...
- 剩余项目级真债（非阻塞）: 详见 [BACKLOG](docs/BACKLOG-intellisource-v1.md)。唯一开放 = **B-074**（P2 远端 infra：产出物已交付，①②⑤ 经真实 CI 闭环 [#130](https://github.com/lync-cyber/intelli-source/pull/130)/[#131](https://github.com/lync-cyber/intelli-source/pull/131)，剩 ③ 置备脚本真机 + ④ GHCR pull 鉴权待首次远端部署）；deploy-ux 评估批 + B-070/B-071 chat 配置均已闭环。延后跟踪：P3 session-splitting（NO-GO）｜P3 KG dangling WARN 76（框架口径噪声，非阻塞，详见上游反馈）
