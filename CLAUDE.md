...
- 当前阶段: backlog-burndown；release gate = approved（B-031 走查 GO，pre-deploy 15-20 全 GO）。D1 Docker 缓存根治已实现+真起栈证伪+提交，[PR #109](https://github.com/lync-cyber/intelli-source/pull/109) 待合并 main
- 当前回归基线: main HEAD 9c1553c（[PR #106](https://github.com/lync-cyber/intelli-source/pull/106) + [PR #107](https://github.com/lync-cyber/intelli-source/pull/107) + [PR #108](https://github.com/lync-cyber/intelli-source/pull/108) 已合并）。D1 Docker 缓存根治在 [PR #109](https://github.com/lync-cyber/intelli-source/pull/109)（分支 `fix/d1-docker-cache-bust`）：Dockerfile `ARG GIT_SHA`+`LABEL` bust src 层（依赖层 CACHED 保留）+ compose build.args + `intellisource up`/`make up` 注入 sha + `--rebuild` 逃生口 + `test_stack.py` 9 用例；受控实验证伪（改 GIT_SHA+改 src+不带 --no-cache → 镜像新 src，uv sync CACHED）；code-review approved_with_notes（R-001~R-004 inline 闭环）；ruff+mypy --strict 267+全量 unit exit 0 绿。前次全门禁基线 integration 160 PASS/1 skip + lint-imports 12/12（本轮未重跑）
