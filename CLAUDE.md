...
- 当前阶段: backlog-burndown；release gate = approved（B-031 走查 GO，pre-deploy 15-20 全 GO）。框架 **0.13.0**（0.12.0→0.13.0 scaffold 刷新；升级误删 gitignored KG store 的 RocksDB CURRENT → doctor KG 两项 FAIL，已从升级备份 `.cataforge/.backups/20260622-045555/kg/store` 恢复并 escalate [CataForge#339](https://github.com/lync-cyber/CataForge/issues/339)；历史升级链 0.4.1→0.12.0 详见 [HISTORY](docs/HISTORY-intellisource-v1.md)），doctor all-pass
- 当前回归基线: main HEAD **7fe5e93**（[#106](https://github.com/lync-cyber/intelli-source/pull/106) ~ [#127](https://github.com/lync-cyber/intelli-source/pull/127) 已合并，闭环详情见 [HISTORY](docs/HISTORY-intellisource-v1.md) / [BACKLOG](docs/BACKLOG-intellisource-v1.md)）；最近合并 [PR #127](https://github.com/lync-cyber/intelli-source/pull/127)（B-074 ②③ 远端 infra：Bash 置备脚本 + systemd 模板 + GHCR registry 推送工作流 + compose pull override + deploy-spec/remote-host-readiness 同步）——纯 infra/docs 改动无 Python/src/tests 表面，本地验证 compose config + bash -n + YAML 全 PASS、镜像名 CI·compose·文档三处一致。无在途特性 PR，工作树干净且与 origin/main 同步。本机 Docker registry-mirror 仅留 `docker.m.daocloud.io`（403 源 `docker.xuanyuan.me` 已在 Docker Desktop UI 删除并 Apply & Restart，`docker info` 复验生效；备份 `daemon.json.bak.20260618`）
- 文档状态: prd / arch（含 API-030）/ dev-plan(主卷+s1~s9+s10) / test-report / deploy-spec（PRE-DEPLOY-WALKTHROUGH 步骤 15-20 已签字）/ backlog = approved；ui-spec = N/A；dev-plan-s8 = draft。test-report + dev-plan-s8r 经 KG 摄取门禁 inline-code 修复（跨文档裸 entity-id 包裹），doctor all-pass；KG store 已 gitignore（派生数据，`cataforge kg rollback <snapshot.nq>` / `kg init` 可重建；旧注 `kg import` 命令已不存在）
- 剩余项目级真债（非阻塞）: 详见 [BACKLOG](docs/BACKLOG-intellisource-v1.md)。唯一开放 = **B-074**（P2 远端 infra：①②③ 产出物已交付 + 本地验证 + 合并 [PR #127](https://github.com/lync-cyber/intelli-source/pull/127)，剩 5 条真实环境验证待首次远端部署，沙盒不可验，详见 BACKLOG）；deploy-ux 评估批 B-072/073/075~079 + B-070/B-071 chat 配置均已闭环。延后跟踪：P3 session-splitting 压缩（2026-06-16 NO-GO，重评触发见 BACKLOG）｜P3 KG dangling WARN 76（TC/CR/SR 纯关系 class 框架口径噪声，修复点在框架侧，escalate [CataForge#292](https://github.com/lync-cyber/CataForge/issues/292)）
