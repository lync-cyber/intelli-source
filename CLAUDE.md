...
- 当前阶段: backlog-burndown — **B-059 + 编码可移植性修复已合入 main（PR #69 + #70）；B-031 唯一 HIGH 阻塞项消除，release gate 待用户 go/no-go 签字**
- 下一步行动: 用户裁定 B-031 release gate go/no-go（HIGH 已消除并合入，剩 B-040 + B-060 MEDIUM observability 非阻塞 → 可签 approved_with_notes）→ B-040/B-060 observability → BGE-M3 本地 embedding 暂缓
- 当前回归基线: **2935 PASS unit** (main HEAD b6713ba, 0 FAIL / 5 deselected)；mypy --strict + ruff + lint-imports 8/8 clean。编码可移植性修复后全量在非 utf-8 locale（Windows gbk）下亦全绿
