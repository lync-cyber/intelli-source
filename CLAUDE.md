...
- 当前回归基线: main HEAD 2a315a4（[PR #106](https://github.com/lync-cyber/intelli-source/pull/106) ~ [PR #119](https://github.com/lync-cyber/intelli-source/pull/119) 已合并；#110 = cataforge 0.9.1 升级 + KG 摄取门禁修复，#111 = KG dangling WARN 处理，#113 = cataforge 0.11.0 升级，#115 = cataforge 0.11.2 升级 + KG 全量重建，#117 = session-splitting 评估 NO-GO + B-070 暴露记账，#118 = B-070/AC-053 chat 压缩缺陷修复（写入端 token-aware + webhooks 同源 + compaction parity helper），#119 = B-071 [chat] 配置对齐立项）。D1 Docker 缓存根治（PR #109 已合并）：Dockerfile `ARG GIT_SHA`+`LABEL` bust src 层（依赖层 CACHED 保留）+ compose build.args + `intellisource up`/`make up` 注入 sha + `--rebuild` 逃生口 + `test_stack.py` 9 用例；受控实验证伪（改 GIT_SHA+改 src+不带 --no-cache → 镜像新 src，uv sync CACHED）；code-review approved_with_notes（R-001~R-004 inline 闭环）；ruff+mypy --strict 267+全量 unit exit 0 绿。本会话 #118 全门禁含 integration 全绿（ruff + mypy --strict 267 + 全量 unit + 全量 integration exit 0）；前次基线 integration 160 PASS/1 skip + lint-imports 12/12
...
- 剩余项目级真债（非阻塞）: 详见 [BACKLOG](docs/BACKLOG-intellisource-v1.md)。开放项 — 部署/分发新手友好度评估（四单元 = 部署/订阅/推送/模板）立 B-072~B-077（[CODE-SCAN-deploy-ux-20260617-r1](docs/reviews/code/CODE-SCAN-deploy-ux-20260617-r1.md)，无新增 P0；B-072/B-073 = P1，B-074/B-075/B-076 = P2，B-077 = P3；用户 2026-06-17 决策 Q1=B+C/Q2=A/Q3=A/Q4=A 已折入修复方向）；B-071/P3 arch `[chat]` 配置段与实现失配（立项研判；B-070 已闭环 [PR #118](https://github.com/lync-cyber/intelli-source/pull/118)，此为其配置对齐后续）；P3 session-splitting 压缩设计(已评估 2026-06-16 NO-GO，重评触发见 BACKLOG)；P3 KG dangling WARN 76 个（TC/CR/SR 纯关系 class 框架口径噪声，已定性，修复点在 CataForge 框架侧，0.11.2 仍复现已聚焦 escalate [CataForge#292](https://github.com/lync-cyber/CataForge/issues/292)）
