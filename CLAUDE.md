...
- 当前阶段: backlog-burndown；release gate = approved（B-031 + 2026-06-08 重构走查全绿）。**2026-06-09 TEI/MCP 新特性首次真起栈走查（PR #102 BGE-M3 + PR #103 MCP gateway）抓到核心缺陷 R-EMB：litellm→TEI embedding 因 ① encoding_format 缺失 ② response.data[0] dict 取值 双 bug 全失效（单测不忠实 mock 漏网，全绿但真路径全断），已 inline 修复 + 回归测试 + 端到端验证（embedding 20/20 写入 1024 维 / semantic·hybrid·MCP 真向量兑现）。修复落地于 feature 分支 fix/tei-embedding-encoding-format（[PR #105](https://github.com/lync-cyber/intelli-source/pull/105)），待合并**。失败注入(步骤18-20)+可观测性(步骤15-17)本次未跑（scope=核心冒烟），prod 发布前仍需补跑
- 当前回归基线: **PR #103（T-MCP-GW f1ad74e）+ PR #104（agent tool-calling 加固 6193a3b）已合并入 main（HEAD b53404b），docs(state) 对账完成**。2026-06-09 走查 R-EMB 修复见 **[PR #105](https://github.com/lync-cyber/intelli-source/pull/105)**（feature 分支 fix/tei-embedding-encoding-format；src/intellisource/llm/gateway/_embed.py + tests/unit/llm/test_gateway_embed_tei.py）。全门禁绿：unit **3426 PASS**（3425+1 回归测试，xdist）+ ruff (check+format) + mypy --strict（_embed.py）；前次基线 integration 160 PASS/1 skip + lint-imports 12/12 KEPT
...
- 剩余项目级真债（非阻塞）: **R-EMB 修复见 [PR #105](https://github.com/lync-cyber/intelli-source/pull/105)，待合并**；观测性 backlog 候选 — TEI healthcheck start_period 偏小（冷启拉 ONNX ~1105s 致瞬态 unhealthy）/ process.py embedding 无 backfill 路径 / /health version 未戳。MCP keyword-only 限制由 T-MCP-GW 消除（R-EMB 修复后真生效）
