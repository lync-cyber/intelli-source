...
- 当前阶段: backlog-burndown；release gate = approved（B-031 走查 GO，pre-deploy 15-20 全 GO）。框架已升级 0.4.1→0.9.1（scaffold 刷新 + IDE 重部署 + KG 摄取门禁修复，[PR #110](https://github.com/lync-cyber/intelli-source/pull/110) 已合并）；KG dangling WARN 处理（API-010/026/029 历史引用 inline-code 豁免 + 76 框架口径噪声定性 + feedback 归因修正）[PR #111](https://github.com/lync-cyber/intelli-source/pull/111) 待合并；D1 Docker 缓存根治已闭环（[PR #109](https://github.com/lync-cyber/intelli-source/pull/109) 已合并）
- 当前回归基线: main HEAD fed4dc5（[PR #106](https://github.com/lync-cyber/intelli-source/pull/106) ~ [PR #109](https://github.com/lync-cyber/intelli-source/pull/109) + [PR #110](https://github.com/lync-cyber/intelli-source/pull/110)（cataforge 0.9.1 升级 + KG 摄取门禁修复）已合并）。D1 Docker 缓存根治（PR #109 已合并）：Dockerfile `ARG GIT_SHA`+`LABEL` bust src 层（依赖层 CACHED 保留）+ compose build.args + `intellisource up`/`make up` 注入 sha + `--rebuild` 逃生口 + `test_stack.py` 9 用例；受控实验证伪（改 GIT_SHA+改 src+不带 --no-cache → 镜像新 src，uv sync CACHED）；code-review approved_with_notes（R-001~R-004 inline 闭环）；ruff+mypy --strict 267+全量 unit exit 0 绿。前次全门禁基线 integration 160 PASS/1 skip + lint-imports 12/12（本轮未重跑）
