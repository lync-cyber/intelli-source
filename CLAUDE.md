...
- 下一步行动: **email 推送链路真起栈走查发现并 inline 修复两真实缺陷（分支 `fix/walkthrough-email-push-defects`，未 commit）：B-061（manual-collect 经 distribute 步骤 `subscription_id=""` → content_not_found → 0 推送，核心）+ B-062（`POST /sources`/`/subscriptions` 同名 upsert UPDATE 路径序列化 MissingGreenlet 500）。重建镜像后端到端复测全绿（distribute 20×sent / push_records 20 sent / mailhog 20 / 幂等去重 / 创建端点 500→201）。** 待用户裁定 commit/PR + **进行中：用真实 SMTP+真实收件地址做一次真实投递测试（需用户提供凭据）**。另立 B-063（RSS collector 瞬时 ConnectError 无重试，P2 观察）。release 已放行，wework 出站 GO/入站 GO-with-condition（前会话结论保留）。**剩余项目级真债 = BGE-M3 本地 embedding 暂缓 + B-061/B-062 待合入**
...
- 最近闭环:
  - **email 推送链路真起栈走查 + B-061/B-062 inline 修复 (本会话, orchestrator 主线程 + Bash 真起栈, 分支 `fix/walkthrough-email-push-defects` 未 commit)** — 用户"触发一次通过邮箱路径的推送，检查流水线是否正确工作"。真起栈（含 mailhog walkthrough profile）跑 manual-collect collect→process→distribute：collect 落 102 raw / process 落 89 processed 均 GO，但 **distribute 实测 20 条内容全 `content_not_found / sent:0`（0 推送）**。根因 **B-061**：distribute step 无 `subscription_id` → 执行器默认 `""` → `facade._load_content_and_subscriptions` 把 `""` 当非法 UUID 返回 `(None,[])` 误判 content_not_found（commit `6d685fb` 仅修 process→distribute id 传递，未修此处，链路仍 0 推送）。附带 **B-062**：`upsert` UPDATE 分支 flush 后未 refresh，`updated_at`(onupdate) expired → 路由 `_serialize_source` 同步访问触发会话外惰性加载 → `MissingGreenlet` → `POST /sources`/`/subscriptions` 同名 500（INSERT 路径靠 asyncpg RETURNING 正常，仅 UPDATE 破；mock 覆盖 `_get_service` 致漏网）。修复：facade falsy `subscription_id`→全部 active 订阅；两 upsert UPDATE flush 后 `refresh` + 3 回归测试。**重建镜像端到端复测**：distribute 20×`sent:1` / push_records 20 行 sent + `w***@example.com` PII 脱敏 / mailhog total=20 真实 HN 标题 / 二次幂等去重 `sent:0,skipped:20` / `POST /sources` 同名 500→201。门禁 tests/unit 全量 PASS + 相关 integration 串行 PASS + mypy --strict + ruff clean。另立 B-063（RSS collector 瞬时 ConnectError 无重试，P2）。详见 [CORRECTIONS-LOG 2026-06-03](docs/reviews/CORRECTIONS-LOG.md)
