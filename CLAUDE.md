...
- 当前回归基线: main HEAD **e90fd84**（[#106](https://github.com/lync-cyber/intelli-source/pull/106) ~ [#125](https://github.com/lync-cyber/intelli-source/pull/125) 已合并，闭环详情见 [HISTORY](docs/HISTORY-intellisource-v1.md) / [BACKLOG](docs/BACKLOG-intellisource-v1.md)）。当前会话 branch `feat/b071-chat-config-settings` → [PR #126](https://github.com/lync-cyber/intelli-source/pull/126)（B-071 chat 配置精简混合收敛 + 状态对账 + BACKLOG/HISTORY 归档轮替）待合并；门禁绿（ruff + mypy --strict 268 + 全量 unit 3705 PASS + chat/webhook integration 23 PASS + doctor all-pass）。**待办**：用户本机 Docker registry-mirror 已改 `~/.docker/daemon.json`（删 403 的 `docker.xuanyuan.me`、留 `docker.m.daocloud.io`，备份 `daemon.json.bak.20260618`），待用户重启 Docker 引擎生效
