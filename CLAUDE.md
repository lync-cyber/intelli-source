...
- 下一步行动: release 已放行，无阻塞项。框架级 backlog 全部移交上游并本地闭环（B-016/B-017/B-018/B-036/B-038 + B-019 → [feedback bundle](docs/feedback/feedback-suggest-framework-batch-20260529.md)，issue 待用户提交至 CataForge 上游）。**B-013/B-040/B-060 + B-020~B-028 均已闭环**（后者经 PR #76：lint-imports/deptry/vulture 退出码 0 + CI lint job 已移除 continue-on-error，违规阻塞 merge）。**剩余开放项 = 项目级真债，刻意保留跟踪（非阻塞）**：仅 P3 B-014（需真 staging 验证 /api/v1/metrics 指标家族）；BGE-M3 本地 embedding 暂缓
- 当前回归基线（本会话实测 @ main HEAD ae379f8）: **2972 PASS unit / 0 FAIL** (`make test-unit` = `-m "not slow" -n auto`，25.7s；含 slow 全量 ~2977) + **163 PASS / 1 skip integration**（CI 验证，含 zhparser migration e5f6a7b8c9d0）；mypy --strict clean (176 files) + ruff clean + lint-imports 8/8 KEPT。分支 = `main`（clean，与 origin 同步）
...
- Backlog 总入口: [docs/BACKLOG-intellisource-v1.md](docs/BACKLOG-intellisource-v1.md) — **release 已放行；剩余全部非阻塞** / 框架级 B-016~B-018/B-036/B-038/B-019 已移交上游闭环 / B-011/B-013/B-020~B-028/B-040/B-060 已闭环 / **项目级真债剩余（保留跟踪）: 仅 B-014（+ BGE-M3 暂缓）**
