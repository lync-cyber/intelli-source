...
def _should_log_session_start(project_dir):
    """Check if session_start should be logged (skip if one exists within 60s)."""
...
def main():
    ...
    # Log session_start event (with dedup to avoid duplicates on autoCompact)
    if _log_event:
        phase = _read_phase(project_dir) if _read_phase else "unknown"
        if _should_log_session_start(project_dir):
            try:
                _log_event(
                    event="session_start",
                    phase=phase,
