...
☐ 待真起栈补签 (B-042 + B-044 已闭环)：2026-05-26 — **B-042 闭环**：`LLMGateway.__init__` 新增 `session_factory` kwarg + `_RetryMixin._emit_call_log()` 统一 cost_tracker（legacy）+ session_factory（生产）双源；chat / stream 切到 helper，**complete() 补 log_call**；`composition.build_llm_gateway` 经 `_build_deps_bundle` 注入 session_factory；单测 10 GREEN。**B-044 闭环**：新增 `LLMSummarizer(BaseProcessor)` + `_NEEDS_LLM_GATEWAY` 类标记 + `_build_processors_from_config(config, llm_gateway=)` 按需注入；`content-process.yaml` 追加末步骤；`_process_execute.repo.create(...)` 持久化 `summary`；单测 11 GREEN。两项 N/A 待用户真起 docker stack 跑 walkthrough 步骤 9 命令链补签：(1) `psql ... SELECT count(*) FROM llm_call_logs WHERE status='success'` ≥ 1（注：实际列名 `input_tokens/output_tokens`，walkthrough 第 475 行 SQL 需同步订正至 B-034）；(2) truncate processed_contents → 重跑 content-process pipeline → `SELECT summary FROM processed_contents WHERE summary != ''` ≥ 1。**B-043 仍未闭环**（chat 路径 cache），步骤 9 第三项（二次执行 cache hit）继续 N/A 跟踪。
