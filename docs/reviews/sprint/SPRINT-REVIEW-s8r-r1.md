...
### [SR-001] MEDIUM (RESOLVED 2026-05-22): dev-plan-s8r 9 任务卡 status 字段未更新（drift）

**resolution**: orchestrator inline 批量修复——9 任务 status: todo → 真实终态（含 final commit + CODE-REVIEW 引用 + EXP-005 端点标注）；T-094 双 status 行清理。grep 验证 11/12 任务 status 字段已反映 git 实际状态。
...
### [SR-006] MEDIUM (DEFERRED 2026-05-22): T-088 lifespan 内 LLMGateway 等资源缺 teardown 清理

**deferral rationale**: orchestrator 在 backlog 处理阶段核查 — `src/intellisource/llm/priority_queue.py` 内部仅 asyncio.Queue + asyncio.Event 数据结构，**无 create_task / background worker coroutine / 持有任何 asyncio.Task**；CircuitBreaker 用 redis（已由 init_redis/close_redis 外部管理）；LLMGateway 无 background coroutine。三类目前都是 passive 数据结构，无 leak 风险。强加 close() API 是 gold-plating。defer 到未来设计变更触发（如 PriorityQueue 引入 background worker pool 时）。
...
### [SR-007] MEDIUM (RESOLVED 2026-05-22): _RawContentFingerprintRepo.record_fingerprint 为 documented no-op

**resolution**: arch-intellisource-v1-modules#§M-002 新增"内容指纹持久化协议"子段，显式声明 fingerprint 写入责任归 collection 层（collector 落库时计算 SHA-256，DB 唯一约束保证去重）；scheduler 层 `FingerprintChecker.record_fingerprint` 是 documented no-op，接口保留以便未来协议迁移。`src/intellisource/scheduler/boot.py:32 _RawContentFingerprintRepo` 类 docstring 同步强化（指向 arch 条款）。148 PASS scheduler/integration 不破坏。
