...
- go/no-go: 核心缺陷 R-EMB 已 inline 修复 + 回归测试 + 端到端验证；新特性 PR #102/#103 修复后功能成立。落地于 [PR #105](https://github.com/lync-cyber/intelli-source/pull/105)（已合并入 main 68d569f）。失败注入(步骤18-20)与可观测性(步骤15-17)本次未跑（scope=核心+新特性冒烟），prod 发布前仍需补跑
...
### 2026-06-11 | orchestrator | B-031 步骤 15-20 重跑 + B-069 version 闭环（用户触发 /start-orchestrator 恢复 → 补跑 pre-deploy 可观测性+失败注入走查）

- 触发信号: /start-orchestrator 恢复时发现 CLAUDE.md §项目状态滞后（实际 PR #106+#107 已合并 main HEAD 13c16d2，状态区仍写 "PR #106 待合并 / HEAD 13b4c13"），先同步状态区+BACKLOG；用户选择补跑 pre-deploy 走查 15-20。已运行栈（api/worker/beat/db/redis/embedding）+ DB 数据保留（1 source Hacker News rss / raw 20 / processed 20 全 embedding / llm_logs 61）
- 环境前置缺陷（部署阻塞 D1，非业务代码缺陷）→ 已修复:
  - **D1 (stale 镜像 BuildKit cache)**: api 容器 lifespan crash — `LLMError: LLM config validation failed: models.{summarize,chat}.fallback_models Extra inputs are not permitted [extra_forbidden]`。根因三证: (a) main HEAD src 支持 fallback_models（`llm_schema.py:34` ModelTaskConfig.fallback_models + `model_config.py` get_fallback_models + gateway `_stream/_complete/_chat` 三处消费）; (b) `config/llm_models.yaml` 带 fallback_models（PR #107 model failover）; (c) 容器内 `_routing.py`=161 行/无 run_with_model_failover vs 宿主 199 行/有 → **镜像 src 是 PR #107 前旧代码**（镜像 created 06-09 21:49 早于 PR #107 合并 06-10 19:24 整一天；`docker compose build` 普通 rebuild 因 Windows Docker Desktop BuildKit 对 `COPY src/` 的 build-context checksum 检测失效而命中 stale layer，镜像时间戳更新但内容旧——`up --build` 后行号仍 161 证实）。`--no-cache` rebuild 后容器 src=199 行/fallback_models present，api lifespan 成功
- 步骤逐项结论（相比 2026-05-29 步骤 15-20 走查，B-040/B-059/B-060 三缺口经 PR #107 agentloop-hardening 全部闭环回归）:
  - **步骤 15 Prometheus+指标 = GO**: `/-/healthy` ✅ / 13 alert rules 加载（api/distribution/health/llm/scheduler 五组，对比 05-29 的 8 条增加）✅ / `/api/v1/metrics` + `/api/v1/system/metrics` 各 7 `# HELP` ✅ / 核心族 http_requests_total/celery_tasks_total/intellisource_health_status/llm_circuit_open present（llm_calls_total/pushes_total 需活动触发后出 sample）
  - **步骤 16 AdaptiveScheduler 长跑 = N/A**（不计划 30min+ 长跑；beat schedule bootstrap 历史步骤 12 已验）
  - **步骤 17 trace_id 跨进程串联 = GO（B-040 闭环回归）**: 实测 trace_id=37358801-... 同现 api inbound（`http request inbound POST /tasks/collect`）+ worker prerun（`celery task prerun run_pipeline`）日志 ✅ + x-trace-id 响应头 ✅。**对比 05-29 的 observability gap（worker_hijack_root_logger 覆盖 formatter + 热路径无 log）已由 PR #107 修复**
  - **步骤 18 DB 失败注入 = GO**: stop db → status=degraded + HTTP 200（db unhealthy last_error InterfaceError，redis/celery up）✅ / 业务 `/api/v1/sources` → 500 ✅ / start db → 自愈 healthy ✅
  - **步骤 19 LLM 上游不可达熔断+降级 = GO（B-060 闭环回归）**: 注入 `DEEPSEEK_API_KEY=sk-invalid` + force-recreate（.env gitignore，已还原）。circuit_state CLOSED→**OPEN** ✅ / chat×5+content-process+search 客户端全 200 无 5xx ✅ / **llm_call_logs error=5（B-060 闭环：失败 LLM 调用现落表，对比 05-29 F3 仅 success）** ✅ / 还原 key + --no-cache rebuild 后 OPEN→HALF_OPEN→（成功 chat probe answer=OK）→**CLOSED** 完整恢复 ✅
  - **步骤 20 Redis 失败注入 = GO（B-059 闭环回归）+ 1 doc-drift**: stop redis → health redis+celery unhealthy ✅ / 非 redis 查询 `/api/v1/sources` → 200 ✅ / **collect → 503 ServiceUnavailable "task broker unavailable; collect not dispatched"（B-059 闭环：fail-fast 不再挂起，对比 05-29 F4 HIGH 的 HTTP 000 15s 挂死）** ✅ / start redis → 自愈 healthy + collect 恢复 202 ✅。doc-drift: walkthrough Pass 标准写 collect "期望 202+fallback"，实际 503 fail-fast 是更正确语义（broker 不可达明确拒绝优于静默丢任务），订正 walkthrough 期望
- B-069 闭环（/health version=0.0.0+unknown → 0.4.6）→ inline 修复（debugger 子代理调度返回异常 0-tool-use，转 orchestrator 主线程 inline，与历史走查 inline 修复一致）:
  - 根因: `version.py` get_version() 用 `importlib.metadata.version("intellisource")`，但 Dockerfile `--no-install-project`（保 dep-layer cache，src 仅经 PYTHONPATH 加载）使部署镜像无包元数据 → PackageNotFoundError → sentinel。version.py 注释 "Never expected in deployed images" 与 Dockerfile 策略矛盾（2026-06-09 走查行 629 已观察，本次正式修复）
  - 修复: `version.py` 增 `_version_from_pyproject()` fallback（tomllib 沿 `Path(__file__).parents` 找 pyproject.toml 读 `[project].version`，metadata→pyproject→sentinel）；`Dockerfile` runtime stage `COPY pyproject.toml`（置 COPY src 前，cache 友好）。+3 单测（metadata 优先断言 9.9.9 / PackageNotFound 走 pyproject 断言 1.2.3 / 双缺失 sentinel）
  - 验证: --no-cache rebuild 后真起栈 `/health` version=**0.4.6** ✅ + fallback_models 持久（lifespan 成功）✅；门禁 version 3/3 + 全量 unit PASS + mypy --strict 267 files + ruff(format+check) 全绿
- 偏差类型: D1=部署操作缺陷（Windows Docker Desktop BuildKit stale COPY cache，非业务代码）；B-069=self-caused（version.py 设计假设与 Dockerfile --no-install-project 策略矛盾）；step20 collect 503 = doc-staleness（实现优于文档期望）
- go/no-go: 步骤 15-20 全 GO（16 N/A）+ B-069 闭环 + D1 修复。**pre_deploy 为 MANUAL_REVIEW_CHECKPOINT，release-gate 最终签字属用户**。本次确认 PR #107 agentloop-hardening 已闭环上次走查（05-29）的 B-040/B-059/B-060 三 observability/resilience 缺口。D1 揭示 Windows Docker Desktop 下部署需强制 `--no-cache` rebuild，立 backlog 根治建议
- 关联: docker/Dockerfile（COPY pyproject.toml）/ src/intellisource/core/version.py / tests/unit/core/test_version.py（B-069）；PR #107 70a03cc（B-040/B-059/B-060 闭环来源：observer.py/signals/scheduler broker_transport_options）；B-069 闭环；D1 BuildKit stale cache 根治建议新立；step20 collect 503 + step15 指标族 doc-drift 并入 walkthrough 订正
