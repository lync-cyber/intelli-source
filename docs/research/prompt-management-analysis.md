...
OpenCode 的 Compaction 系统（对话总结 + 工具输出裁剪 + 溢出检测）非常精巧，但 **完整系统不适合 IntelliSource**:

- IntelliSource 的 LLM 调用主要是 **无状态的单轮处理**（提取、分类、摘要），不是多轮对话
- 唯一的多轮场景是即时问答 (ChatSession)

**v1 实际采纳方案**: 基于 PRD AC-053 的明确要求（"通过 LLM 压缩管理上下文"），v1 实现了简化版 compaction 机制——token 预算控制 + 超限时 LLM 摘要压缩旧消息。这比 OpenCode 的完整系统（工具输出裁剪 + 溢出检测 + 独立 compaction agent）轻量得多，仅针对 ChatSession 的多轮对话场景，不涉及工具输出裁剪和复杂的溢出检测机制
...
| P1 | 简化版对话压缩 (§3.6) | 中 | 已采纳简化版（token 预算 + LLM 摘要），满足 PRD AC-053 |
