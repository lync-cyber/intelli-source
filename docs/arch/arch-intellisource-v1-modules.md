...
- **内部关键组件**:
  ...
  - `SessionCleaner` — 过期会话清理器，由 Celery Beat 定时任务每小时检查并清理超过 24 小时无活跃的会话（E-011 清理策略）

### M-009: 存储与检索模块 (storage)

- **职责**: 管理结构化数据和向量数据的持久化存储，提供统一的数据访问层和混合检索能力
- **映射功能**: F-012（存储与混合检索）
- **对外接口**: API-014（内容列表）, API-015（内容详情）, API-016（聚类列表）, API-030（删除单条内容）, API-031（批量删除内容）, API-032（存储统计查询）
- **依赖模块**: 无（基础设施模块）
- **内部关键组件**:
