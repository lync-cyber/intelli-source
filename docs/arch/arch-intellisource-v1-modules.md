...
- **内部关键组件**:
  ...
  - 每个 LLM 处理器均实现降级逻辑（AC-021, AC-027, AC-049），降级映射表见 arch#§5.3
...
- **内部关键组件**:
  ...
  - `CircuitBreaker` — 熔断器实现（AC-029），具体参数见 arch#§5.3 熔断机制
  - `FallbackManager` — 降级管理器，LLM 失败时自动切换（AC-030），降级映射表见 arch#§5.3
