...
- **职责**: 管理信息源的声明式配置，提供配置的 CRUD、校验、热加载和版本管理能力。数据库为运行时权威来源，配置文件为初始化种子（见 prd#§2 F-001 备注）
- **映射功能**: F-001（信息源声明式配置）
...
- **内部关键组件**:
  ...
  - **错误隔离策略**: 单个处理器失败时，若该处理器标记为 `optional=true` 则跳过继续（记录 WARNING），否则整个管道标记为 failed。批处理模式中单条内容失败不影响同批其他内容
...
- **内部关键组件**:
  ...
  - LLM 调用优先级隔离（AC-032）：FastAPI 路由中的 LLM 调用直接使用 httpx async（用户交互优先级），Celery task 中的 LLM 调用经过 Redis 令牌桶限流（后台优先级），两条路径共享熔断器状态
...
- **内部关键组件**:
  ...
  - `WorkflowEngine` — 工作流引擎，v1 仅支持线性步骤编排（采集→处理→分发顺序执行），不支持并行分支或条件路由（AC-063）
...
- **内部关键组件**:
  ...
  - `AlertManager` — 告警管理器（AC-060）；v1 通过 structlog 输出 CRITICAL 级别日志实现告警，用户可通过外部日志监控工具（如 Grafana Loki）配置告警规则；不内置告警渠道推送
