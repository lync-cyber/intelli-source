...
- §5 非功能架构 → §5.1 性能, §5.2 安全, §5.3 错误处理, §5.4 数据运维
- §6 目录结构
...
- v1 不提供自动数据清理功能（prd#§4），用户需手动管理存储空间
- LLMCallLog（E-007）按月分区，超过 3 个月的分区可通过 `DROP TABLE` 归档或删除
- ChatSession（E-011）超过 24 小时无活跃的会话，可通过 CLI 命令 `intellisource cleanup sessions` 清理（T-045 实现）
- RawContent / ProcessedContent 的历史数据清理由用户根据存储容量自行决定，建议通过 `published_at` 时间范围删除

**Embedding 模型切换迁移方案**:

- 切换 embedding 模型时（如从 1536 维切换到其他维度），需执行以下步骤:
  1. 修改 M-001 配置管理的 `embedding_dimension` 配置项
  2. 执行 Alembic 迁移修改 E-004 embedding 和 E-005 centroid 的 VECTOR 维度
  3. **全量重新生成 embedding**: 对所有 ProcessedContent 和 ContentCluster 记录重新调用新模型生成向量，通过 CLI 命令 `intellisource reindex embeddings` 批量执行（T-045 实现）
  4. 迁移期间系统可正常运行，未重新生成 embedding 的内容暂时不参与向量检索
- 不支持新旧维度共存（pgvector 的 VECTOR 类型维度固定），因此切换为一次性全量操作
