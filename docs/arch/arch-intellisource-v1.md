...
| API-030 | 删除单条内容 | M-009 | DELETE | /api/v1/contents/{id} |
| API-031 | 批量删除内容 | M-009 | POST | /api/v1/contents/batch-delete |
| API-032 | 存储统计查询 | M-009 | GET | /api/v1/storage/stats |
...
| API 路径（例外） | 操作型端点（search）和统计端点（stats）可使用单数或动词形式 | `/api/v1/search`, `/api/v1/llm/stats` |
