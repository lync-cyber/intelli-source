...
- LLM 调用结果缓存：相同内容指纹 + 相同处理类型的 LLM 调用结果缓存，TTL 由 M-001 AppSettings.llm.result_cache_ttl 配置（默认 24h），减少重复调用
- 检索结果缓存：高频查询关键词缓存，TTL 由 M-001 AppSettings.search.search_cache_ttl 配置（默认 5min，短 TTL 保证时效性）
...
- 默认每页由 M-001 AppSettings.pagination.default_page_size 配置（默认 20 条），最大由 max_page_size 配置（默认 100 条）
...
- LLM 服务调用采用熔断器模式
- 连续失败次数达到 M-001 AppSettings.llm.circuit_breaker_threshold（默认 5 次）触发熔断（Open 状态），停止调用 circuit_breaker_cooldown 秒（默认 60s）
- 冷却期后进入半开状态（Half-Open），允许 1 次试探调用
...
- 降级映射表:
  ...
  | 内容重排序 | 默认时间排序 |
  | 引导语生成 | 无引导语 |
  | 对话上下文压缩 | truncate 模式（丢弃最早对话，保留最近 max_rounds 轮） |
