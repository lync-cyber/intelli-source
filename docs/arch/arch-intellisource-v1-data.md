...
| context | JSONB | NOT NULL, DEFAULT '{"summary": null, "messages": []}' | 对话上下文，结构: {summary: string\|null, messages: [{role: "user"\|"assistant", content: string, timestamp: datetime}]}。truncate 模式下仅使用 messages 数组（保留最近 N 轮，N 由 M-001 AppSettings.chat.max_rounds 配置，默认 5）；compress 模式下 summary 存储 LLM 生成的历史对话语义摘要，messages 仅保留最近 compress_threshold 轮原始对话。不单独建表 |
...
**清理策略**: 超过 `session_timeout_hours` 小时（M-001 AppSettings.chat 配置，默认 24）无活跃的会话自动清理
