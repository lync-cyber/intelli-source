...
IntelliSource 由 7 个核心容器组成，通过 Docker Compose 单主机编排；Prometheus 为可选 observability 组件（`--profile observability` 启用）。

#### Embedding 服务（HuggingFace TEI / BGE-M3）

向量 embedding 由独立的 `embedding` 容器提供，基于 HuggingFace Text Embeddings Inference（TEI），加载 `BAAI/bge-m3` 模型，输出 1024 维向量。TEI 在容器内监听端口 80，对外暴露 OpenAI 兼容端点 `http://embedding/v1`（路径 `/v1/embeddings`）。

- **CPU 部署（默认）**：镜像 `ghcr.io/huggingface/text-embeddings-inference:cpu-1.6`（可通过 `IS_EMBEDDING_IMAGE` 覆盖）。
- **GPU 部署**：`IS_EMBEDDING_IMAGE` 设为 CUDA tag（如 `ghcr.io/huggingface/text-embeddings-inference:1.6`），同时使用 GPU override：`docker compose -f docker-compose.yml -f docker-compose.gpu.yml up`。
- **软依赖**：api / worker 对 `embedding` 采用 `service_started` 软依赖；embedding 未就绪时 embed 调用失败，应用自动降级为 zhparser 全文检索（keyword-only），不阻塞启动。
- **模型缓存**：命名卷 `embedding_data` 持久化模型权重，重启无需重新下载。
- **验证**（归 PRE-DEPLOY-WALKTHROUGH B-031）：起栈后 `curl http://localhost/health`（宿主映射端口按 walkthrough 确认）；`/v1/embeddings` 端点路由及镜像 tag 以当前 TEI 文档为准，walkthrough 时确认实际可用性。
...
│                                                                          │
│  ┌──────────────────────────────────────────────────────────┐            │
│  │  embedding (TEI / BGE-M3)   port 80 (内部)               │           │
│  │  http://embedding/v1/embeddings   dim=1024               │           │
│  │  CPU default; GPU via docker-compose.gpu.yml override    │           │
│  └──────────────────────────────────────────────────────────┘            │
│                                                                          │
...
| embedding | 80 | 不对外暴露 | TEI OpenAI 兼容端点；仅 Docker 内部网络（`http://embedding/v1`）|
...
| 维度 | dev | staging | prod |
|------|-----|---------|------|
| 镜像 tag（本地构建）| `intellisource:dev` | `intellisource:${GIT_SHA_SHORT}-staging` | — |
| 镜像 tag（GHCR registry）| — | `ghcr.io/lync-cyber/intellisource:<sha>` | `ghcr.io/lync-cyber/intellisource:<sha>` 或 `vX.Y.Z` |
| DB 镜像 tag（GHCR）| — | `ghcr.io/lync-cyber/intellisource-db:pg16-pgvector-zhparser` | 同 staging |
...
| 变量名 | 必填 | 默认值 | 敏感度 | 说明 |
|--------|------|--------|--------|------|
| `IS_EMBEDDING_API_BASE` | 否 | `""` (降级) | LOW | TEI 端点 base URL；compose 内默认 `http://embedding/v1`；留空则降级为 zhparser FTS keyword-only，无硬失败 |
| `IS_EMBEDDING_API_KEY` | 否 | `""` (gateway 内兜底 `"tei"`) | LOW | TEI 无鉴权时填任意非空字符串即可；使用有鉴权的远程 embedding 服务时按需配置 |
| `IS_EMBEDDING_DIMENSION` | 否 | `1024` | LOW | 向量维度；须与 DB `VECTOR(1024)` 列（迁移 `g0h1i2j3k4l5`）及实际模型输出维度一致 |
| `IS_EMBEDDING_IMAGE` | 否 | `ghcr.io/huggingface/text-embeddings-inference:cpu-1.6` | LOW | TEI 容器镜像；GPU 部署时覆盖为 CUDA tag，同时使用 `docker-compose.gpu.yml` override |
...
sbom          →  trivy 生成 SBOM（CycloneDX JSON），attached 为 workflow artifact
scan          →  trivy 镜像漏洞扫描，HIGH/CRITICAL 阻塞 push（§1.4 门禁）
build+push    →  .github/workflows/publish-images.yml；push main / tag v* 触发
                  App:  ghcr.io/lync-cyber/intellisource:<sha> + latest + vX.Y.Z
                  DB:   ghcr.io/lync-cyber/intellisource-db:pg16-pgvector-zhparser + <sha>
deploy-staging→  docker compose -f docker/docker-compose.yml \
                              -f docker/docker-compose.registry.yml pull && up -d --no-build
...
```bash
# 1. 启动基础设施 + 数据库迁移（含 embedding 服务）
docker compose -f docker/docker-compose.yml up -d db redis migrate embedding
docker compose -f docker/docker-compose.yml wait migrate

# 2. 启动应用服务（embedding 首次启动需下载模型，api/worker 软依赖不阻塞）
...
使用 GHCR prebuilt 镜像（`docker/docker-compose.registry.yml` override）可实现秒级回滚，无需重新构建：

```bash
# 1. 确认目标回滚 sha（从 GHCR 或 git log 查找）
...
# 3. 回滚 api 和 worker 到目标 sha（秒级，仅切换镜像标签）
...
IS_IMAGE_TAG=<prev-sha> docker compose \
    -f docker/docker-compose.yml \
    -f docker/docker-compose.registry.yml \
    up -d --no-build --no-deps --force-recreate beat
```

本地构建模式回滚（不使用 registry，需重新编译）：

```bash
# 检出旧 tag 源码后重新构建（db 镜像含 zhparser 编译，耗时 3-10 分钟）
PREV_TAG="<上一个稳定 git tag>"
git checkout "${PREV_TAG}"
docker compose -f docker/docker-compose.yml build api worker
docker compose -f docker/docker-compose.yml up -d --no-deps --force-recreate api worker beat
```
