...
# ---------------------------------------------------------------------------
# 7. truncate_fallback (pure, non-LLM digest; LLM path lives in the agent layer
#    — see tests/unit/agent/test_summarize_cluster.py)
# ---------------------------------------------------------------------------
class TestTruncateFallback:
    """Tests for the pure first-3-sentence truncation digest."""
    ...
    def test_basic_cluster(self) -> None:
        ...
        result = truncate_fallback(contents)
    ...
    def test_empty_cluster_contents(self) -> None:
        result = truncate_fallback([])
        assert result.title == ""
    ...
    def test_summary_ends_with_period(self) -> None:
        result = truncate_fallback([{"title": "T", "body_text": "No period at end"}])
