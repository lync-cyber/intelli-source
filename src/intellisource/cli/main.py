...
def pipeline_show(
    ...
    url = f"{_base_url()}/api/v1/pipelines/{name}"
    resp = _http(lambda: httpx.get(url, headers=_get_headers()))
    if resp.status_code == 404:
        typer.echo("Not found")
        raise typer.Exit(code=1)
...
def pipeline_run(
    ...
    resp = _post_json(f"/api/v1/pipelines/{name}/run", payload)
    if resp.status_code == 404:
        typer.echo("Not found")
        raise typer.Exit(code=1)
    if resp.status_code >= 400:
...
def pipeline_create(
    ...
    resp = _post_json("/api/v1/pipelines", payload)
    if resp.status_code >= 400:
        try:
            detail = _error_message(resp)
        except Exception:
...
def pipeline_update(
    ...
    if not payload:
        ...
        raise typer.Exit(code=2)
    url_path = f"{_base_url()}/api/v1/pipelines/{name}"
    resp = _http(lambda: httpx.patch(url_path, json=payload, headers=_get_headers()))
    if resp.status_code == 404:
        typer.echo("Not found")
        raise typer.Exit(code=1)
...
def _chat_once(
    ...
    resp = _post_json("/api/v1/agent/chat", payload)
    if resp.status_code >= 400:
        try:
            detail = _error_message(resp)
        except Exception:
...
def topic_show(
    ...
    url = f"{_base_url()}/api/v1/topics/{topic_id}"
    resp = _http(lambda: httpx.get(url, headers=_get_headers()))
    if resp.status_code == 404:
        typer.echo("Not found")
        raise typer.Exit(code=1)
